...
def start():
    ...
    while True:
        ...
        for piece in grid:
            ...
            y = measure + y_measure
            if piece == 'O':
                pygame.draw.circle(screen, (0, 0, 0), (x + (piece_pos[0] + 1) * grid_size, y + (piece_pos[1] + 1) * grid_size), grid_size * 0.75, 3)
            elif piece == 'X':
                pygame.draw.line(screen, (0, 0, 0), (x + grid_size * 0.25 + x_measure, y + grid_size * 0.25 + y_measure), (x + grid_size * 1.75 + x_measure, y + grid_size * 1.75 + y_measure), 4)
                pygame.draw.line(screen, (0, 0, 0), (x + grid_size * 1.75 + x_measure, y + grid_size * 0.25 + y_measure),  (x + grid_size * 0.25 + x_measure, y + grid_size * 1.75 + y_measure), 4)
            elif clicked and x + x_measure < mouse[0] < x + grid_size * 2 + x_measure and y + y_measure < mouse[1] < y + grid_size * 2 + y_measure:
                grid[1 - turn] = 'O' if turn else 'X'
                turn = 1 - turn
                if check(1 - turn):
